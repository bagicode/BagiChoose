/*
---
MooTools 1.3: the javascript framework
...
i=(i)?j+"&"+i:j;d="post";}if(this.options.urlEncoded&&["post","put"].contains(d)){var f=(this.options.encoding)?"; charset="+this.options.encoding:"";this.headers["Content-type"]=this.options.content_type+""+f;
...
Number.implement({
    ...
    */
    ...
    numberFormat: function(decimals, dec_point, thousands_sep) {
        decimals = Math.abs(decimals) + 1 ? decimals : 4;
        dec_point = dec_point || Culture.Current.decimalSeparator || '.';
        thousands_sep = thousands_sep || Culture.Current.thousandsSeparator || ',';
        ...
        var returnVal = (matches[1] ? matches[1] : '') + (remainder ? matches[2].substr(0, remainder) + thousands_sep : '') + matches[2].substr(remainder).replace(/(\d{3})(?=\d)/g, "$1" + thousands_sep) +
				(decimals ? dec_point + (+matches[3] || 0).toFixed(decimals).substr(2) : '');


        //var matches2 = /((\d*\,\d?[0])|(\d*\,[1-9]*))/.exec(returnVal);

        var matches2 = Culture.Current.decimalRegex.exec(returnVal);
        if (matches2) {
            //            var addthis = Culture.Current.decimalSeparator + '00';

            //            if (matches2[1] == '') {
            //                returnVal = returnVal.replace(matches2[0], '') + addthis;
            //            }
            //            else {
            //               returnVal = returnVal.replace(matches2[0], '') + matches2[1];
            //            }
            returnVal = matches2[0];
        }
...
Date.Month = { 'Jan': 1, 'Feb': 2, 'Mar': 3, 'Apr': 4, 'May': 5, 'Jun': 6, 'Jul': 7, 'Aug': 8, 'Sep': 9, 'Oct': 10, 'Nov': 11, 'Dec': 12 };